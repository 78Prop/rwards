...
import { MiningServer } from './mining-protocols.js';
...
app.use(expressSession({
  secret: 'kloudbugscafe-mining-secret',
  resave: false,
...
const kloudBugsCafeServer = new MiningServer(config.pools.kloudBugsCafe.port);
const teraSocialJusticeServer = new MiningServer(config.pools.teraSocialJustice.port);
const teraTokenServer = new MiningServer(config.pools.teraToken.port);
...
const miningPools = [
  {
    ...
    name: config.pools.kloudBugsCafe.name,
    url: `stratum+tcp://${config.host}:4004`, // Fixed port to match config
    status: 'connected',
    hashRate: 450,
    address: config.pools.kloudBugsCafe.address,
    username: 'Kloudbugs7',
    managed: true,
    fees: config.pools.kloudBugsCafe.fee,
    connectedRigs: 4,
    server: kloudBugsCafeServer
  },
  {
    id: 'tera-social-justice-pool',
    name: config.pools.teraSocialJustice.name,
    url: `stratum+tcp://${config.host}:4005`, // Fixed port to match config
    status: 'connected',
    hashRate: 490,
    address: config.pools.teraSocialJustice.address,
    username: 'Kloudbugs7',
    managed: true,
    fees: config.pools.teraSocialJustice.fee,
    connectedRigs: 4,
    server: teraSocialJusticeServer
  },
  {
    id: 'tera-token-pool',
    name: config.pools.teraToken.name,
    url: `stratum+tcp://${config.host}:4006`, // Fixed port to match config
    status: 'connected',
    hashRate: 470,
    address: config.pools.teraToken.address,
    username: 'Kloudbugs7',
    managed: true,
    fees: config.pools.teraToken.fee,
    connectedRigs: 4,
