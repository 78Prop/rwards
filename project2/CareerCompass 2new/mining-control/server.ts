...
const WS_PORTS = [5001, 5002, 5003, 5004, 5005];
...
app.get('/api/operations', (req, res) => {
  ...
  const USER_BTC_ADDRESS = process.env.USER_BTC_ADDRESS || 'bc1qj93mnxgm0xuwyh3jvvqurjxjyq8uktg4y0sad6';
  ...
  res.json({
    ...
    userBtcAddress: USER_BTC_ADDRESS,
    totalDailyRevenue: summary.totalRevenue,
    allProfitsToUser: {
      percentage: 100,
      dailyAmount: summary.totalRevenue,
      btcAddress: USER_BTC_ADDRESS,
      message: "All mining profits directed to user BTC wallet"
    }
  });
});
  ...
  res.send(`
    ...
    <body>
        ...
        <script>
            ...
            const ws = new WebSocket('ws://localhost:' + wsPort + '/mining-ws');
...
// Start server with WebSocket retry logic
async function startServer() {
  try {
    // Start HTTP server first
    await new Promise<void>((resolve, reject) => {
      server.listen(PORT, HOST, () => {
        console.log(`🚀 KLOUDBUGS Mining Control Center running on ${HOST}:${PORT}`);
        ...
        if ((error as any).code === 'EADDRINUSE') {
          console.error(`Port ${PORT} is in use. Please specify a different port using the PORT environment variable.`);
        }
        reject(error);
      });
    });
    ...
    const success = await initializeWebSocket();
    if (!success) {
      console.error('Failed to start WebSocket server after trying all ports');
      process.exit(1);
    }
