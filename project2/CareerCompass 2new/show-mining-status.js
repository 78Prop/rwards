...
minersDb.all('SELECT id, name, pool FROM miners', (err, rows) => {
  ...
  // Share Submissions
  accountsDb.all('SELECT rigId, poolId, shares, confirmedBalance FROM miner_balances', (err2, balances) => {
    if (err2) throw err2;
    console.log('\n[SHARE SUBMISSIONS]');
    console.log('----------------------------------------');
    balances.forEach((bal, i) => {
      console.log(`Miner-${(i+1).toString().padStart(2, '0')} submitted ${bal.shares || 0} shares to ${bal.poolId} - Earned: ${bal.confirmedBalance || '0.00000000'} BTC`);
    });
    minersDb.close();
    accountsDb.close();
  });
